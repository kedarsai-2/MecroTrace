...
// Reuse the same animated particle background pattern as the Trader login
...
const ContactPortalLoginPage = () => {
  ...
  // Phone + OTP state (phone-only, no email)
  ...
  // Email validation (same as Trader login)
  ...
  // Phone validation (same as Trader login; OTP for mobile only)
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      toast.success('OTP sent', {
        description: 'Please check your phone for the 4-digit OTP.',
      });
    } catch (e: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // In phone mode, the primary action is either "Send OTP" or "Verify & Sign In"
    if (!otpSent) {
      await handleSendOtp();
    } else {
      ...
      <div className="relative z-10 flex-1 flex flex-col lg:flex-row min-h-0">
        ...
        <div className="hidden lg:flex lg:w-[55%] items-end p-12">
          ...
          >
            ...
            <p className="text-white/70 text-lg max-w-md">
              Login as Seller, Buyer, or Broker to view your arrivals, statements, and purchases.
            </p>
          </motion.div>
        </div>
        ...
        <main className="flex-1 flex flex-col min-h-0 overflow-y-auto overscroll-y-contain">
          ...
          <div className="flex justify-end px-5 pt-[max(1rem,env(safe-area-inset-top))]">
            <Link
              to="/login"
              className="text-sm font-semibold text-white/90 underline min-h-[44px] flex items-center"
            >
              Go to Trader Login
            </Link>
          </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center px-6">
            ...
            >
              <h1 className="lg:hidden text-2xl sm:text-3xl font-bold text-white mb-1 drop-shadow-lg">
                Contact Portal
              </h1>
              <h2 className="hidden lg:block text-2xl sm:text-3xl font-bold text-white mb-1 drop-shadow-lg">
                Contact Portal
              </h2>
              <p className="text-white/70 text-sm sm:text-base">
                Sign in as Seller, Buyer, or Broker to continue.
              </p>
            </motion.div>
            ...
            <AnimatePresence mode="wait">
              {loginMode === 'phone' ? (
                ...
                >
                  <div>
                    <label htmlFor="portal-login-phone" className="sr-only">
                      Phone number
                    </label>
                    <div className="relative">
                      <Phone
                        className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-blue-800/50"
                        aria-hidden="true"
                      />
                      <Input
                  ...
                  {otpSent && (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                      <label htmlFor="portal-login-otp" className="sr-only">
                        OTP
                      </label>
                      <div className="relative">
                        <KeyRound
                ...
                <motion.form
                  ...
                  aria-label="Contact portal email sign in form"
                >
                  <div>
                  ...
                  <div>
                    <label htmlFor="portal-login-password" className="sr-only">
                      Password
                    </label>
                    <div className="relative">
                      <Lock
...
export default ContactPortalLoginPage;
